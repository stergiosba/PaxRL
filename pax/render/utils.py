...
class CircleEntity(pg.shapes.Circle):
    def __init__(
        ...
        if neighbourhood:
            self.neighborhood = pg.shapes.Circle(
                x,
                y,
                radius=neighborhood_radius,
                color=(color[0], color[1], color[2], 45),
                batch=batch,
            )

        if heading:
            self.heading = pg.shapes.Line(
                x, y, x, y, 2, color=(255, 0, 0, 255), batch=batch
            )
...
class TriangleEntity(pg.shapes.Triangle):
    def __init__(
        self, x, y, radius, neighborhood_radius, color=..., batch=None, group=None
    ):
        super().__init__(
            x,
            y - radius / 2,
...
class ScriptedEntity(CircleEntity):
    def __init__(
        ...
        super().__init__(
            x, y, radius, neighborhood_radius, segments, color, batch, group
        )
