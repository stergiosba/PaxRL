...
from jax import jit, lax, devices, debug
...
class Environment(eqx.Module):
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[chex.Array, EnvState]:
        ...
        init_X_scripted_entities = jrandom.uniform(key, minval=350, maxval=650, \
            shape=(self.n_scripted_entities,2))
        init_X_dot_scripted_entities = jrandom.uniform(key, minval=-1, maxval=1, \
            shape=(self.n_scripted_entities,2))
        
        init_X_agents = jrandom.uniform(key, minval=50, maxval=150, shape=(self.n_agents,2))
        init_X_dot_agents = jrandom.uniform(key, minval=-1, maxval=1, shape=(self.n_agents,2))
    ...
    def single_rollout(self, key):#, policy_params):
        ...
        def policy_step(state_input, tmp):
            ...
            #else:
            scripted_action = jnp.ones([self.n_scripted_entities,2])
    ...
    def step(self, state:EnvState, action):
        ...
        X = state.X + 100*dt*X_dot/la.norm(X_dot)
