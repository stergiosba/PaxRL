...
def reynolds_jax(leader: int, X: chex.Array, X_dot: chex.Array) -> Tuple[chex.Array, int]:
    ...
    return total_mask*(w_c*cohesion+ w_a*alignment) + neighbors_mask*w_s*separation


def batch_rollout(keys: random.KeyArray,X,X_dot) -> chex.Array:
    ...
    """
    batch_rollout = vmap(single_rollout, in_axes=(0,None,None))
    return batch_rollout(keys,X,X_dot)

def single_rollout(key: chex.PRNGKey, X, X_dot):
    def exper(key, tmp):
        key,_ = random.split(key, 2)
        A = reynolds_jax(0,X,X_dot)
        carry = key
        y = A
...
reynolds_jax.lower(0, X,X_dot).compile()
#%%
...
for n in range(1,501,10):
    key = random.PRNGKey(n)
    key_v = random.PRNGKey(n*m-32)
    X = random.uniform(key,shape=(n,2))
    X_dot = random.uniform(key_v,shape=(n,2))
